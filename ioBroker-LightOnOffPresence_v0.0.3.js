...
let arrGroups = [
    //Büro
    {
        ...
        pathDimmUp: ['hm-rpc.1.00171A499D46DC.32.PRESS_LONG'],  //Pfad zum Taster für Hochdimmen
        pathDimmDown: ['hm-rpc.1.00171A499D4685.1.PRESS_LONG'], //Pfad zum Taster für Runterdimmen
    ...
    /*
    {
        description: 'Wohnzimmer',                              //Beschreibung der Gruppe. Notwendig zur Erzeugung der Datenpunkte
        ...
        pathDimmUp: ['hm-rpc.1.00171A499D46DC.14.PRESS_LONG'],  //Pfad zum Taster für Hochdimmen
        pathDimmDown: ['hm-rpc.1.00171A499D46DC.15.PRESS_LONG'], //Pfad zum Taster für Runterdimmen
        pathBWM: ['zigbee.0.00158d0004514986.occupancy'],       //Pfad zum Bewegungsmelder
        pathLux: 'zigbee.0.00158d0004514986.illuminance',       //Pfad zum Lichtsensor in Lux
        timerMotion: 10,                                        // Sekunden: Zeit, nachder das Licht ausgeschaltet wird, sofern keine Bewegung mehr erkannt wird.
        timerSwitch: 3600,                                      // Sekunden >0: Licht wird automatisch ausgeschaltet, sofern keine Bewegung mehr erkannt wird (Rückfallebene) 
        lights: [4],                                            // Leuchten, welche zur Gruppe gehören
        lockBWM: true,                                          // Sperre für Bewegungsmelder, wenn das Licht manuell per Taster oder Datenpunkt eingeschaltet wurde
        defaultSwitch: true,                                    // true: Lampen werden mit Schalter auf Default Helligkeit gesetzt // false: Lampen werden nach Zeitplan gesetzt
        keyStrokes: 1,                                          // 1 = Nur ein Tastendruck EIN wird gewertet. >1 = Es werden entsprechend der Zahlt gewertet
    },
...
let arrLights = {
    ...
    3: {
        name: 'Wohnzimmer Spot 1',
        pathControl: 'zigbee.0.00158d0002f3cfa7.brightness',    // Pfad zum Datenpunkt, mitdem die Lampe geschaltet wird. (Brightness oder State)
        pathColortemp: 'zigbee.0.00158d0002f3cfa7.colortemp',   // Pfad zum Datenpunkt, mitdem die Lampe geschaltet wird. (Brightness oder State) - sofern vorhanden
        pathTransistion: 'zigbee.0.00158d0002f3cfa7.transition_time',   // Pfad zum Datenpunkt, für die Transition Time.
        pathColor: 'zigbee.0.00158d0002f3cfa7.color',           // Pfad zum Datenpunkt, für die Farbe. - sofern vorhanden
        defaultLevel: 70,                                       // Default Level-Wert beim Einschalten, sollte kein Zeitplan zutreffen
        defaultColorTemp: 251,                                  // Sofern ein Wert gesetzt ist, Farbtemperatur - sofern vorhanden
...
let arrSchedules = {
    // Zeitplan 1
	1: {from: '06:00', to: '22:00', brightness: 70, colorTemp: 251, color: '', state: true, name:"Tagsüber von 6-22 Uhr: Helligkeit 70"  },
	// Zeitplan 2
	2: {from: '22:00', to: '06:00', brightness: 5, colorTemp: 251, color: '', state: true, name:"Nachts von 22-6 Uhr Helligkeit 5" },
	// Zeitplan 3
	3: {from: '05:00', to: '22:00', brightness: 70, colorTemp: 251, color: '', state: true, name:"Tagsüber von 6-22 Uhr: Helligkeit 70" },
	// Zeitplan 4
	4: {from: '22:00', to: '06:00', brightness: 0, colorTemp: 251, color: '', state: false, name:"Nachts von 0 - 6 Uhr AUS" },
};
...
for (const i in arrLights) {
    ...
    arrLights[i].pathControlType = getObject(arrLights[i].pathControl).common.type;
    ...
    arrLights[i].setLightOn = async function(group){
        if (group.buttonPress == this.buttonPress) {
            ...
            if (this.pathControl) {
                await setStateAsync(this.pathControl, (this.pathControlType == 'number') ? this.defaultLevel : true)
    ...
    arrLights[i].setLightOff = async function(group){
        ...
        if (this.pathControl) {
            await setStateAsync(this.pathControl, (this.pathControlType == 'number') ? 0 : false);
    ...
    arrLights[i].setLightDimmUp = async function(){
        if (this.pathControl && this.pathControlType == 'number') {
    ...
    arrLights[i].setLightOnSchedule = async function(group){
        for (const y in this.schedules) {
            ...
            if (compareTime(schedule.from, schedule.to, "between", null)) {
                ...
                if (this.pathControl) {
                    await setStateAsync(this.pathControl, (this.pathControlType == 'number') ? schedule.brightness : schedule.state)
    ...
    arrLights[i].setLightEmergencyOn = async function(group){
        if (this.emergency >= 1) {
            ...
            if (this.pathColortemp) await setStateAsync(this.pathColortemp, this.defaultColorTemp)
            if (this.pathControl) {
                await setStateAsync(this.pathControl, (this.pathControlType == 'number') ? this.emergencyLevel : true)
    ...
    arrLights[i].setLightEmergencyOff = async function(group){
        if (this.emergency >= 1) {
            ...
            if (this.pathControl) {
                await setStateAsync(this.pathControl, lastValue)
...
async function createTrigger(arrGroups, lights, schedules) {
    arrGroups.forEach(function async(objTemp) {
        ...
        if(objTemp.pathOn) {
            on({ id: objTemp.pathOn, val: true }, function async(obj) {
                ...
                for (const n in objTemp.lights) {
                    let light = lights[objTemp.lights[n]];
                    (objTemp.defaultSwitch) ? light.setLightOn(objTemp) : light.setLightSchedule(objTemp)
                }
        ...
        if(objTemp.pathBWM) {            
            on({ id: objTemp.pathBWM, change: "any", ack: true}, async function (obj) {  // BWM
                ...
                if(!objTemp.switch){
                    ...
                    let luxValue = 0;
                    luxValue = luxVal.val;
                    if(extLogging) console.log(`${scriptname}: Aktuelle Lux-Wert: ${luxValue}`);
...
async function setAck(obj) {
    if (!obj.state.ack) setState(obj.id, obj.state.val, true);
}
